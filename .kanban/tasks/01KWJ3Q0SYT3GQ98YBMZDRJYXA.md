...
- actor: wballard
  ...
  text: |-
    ...
    Updated every call site: internal cross-calls in Trigram.swift, Tokenizer.swift's own `splitRun`, and all call sites in Tests/CodeContextKitTests/RankerTests.swift (11 tokenize, 6 charTrigrams, 7 dice call sites). Also fixed stale doc-comment references to the old `(_:)`/`(_:_:)` signatures in Trigram.swift and Hit.swift.

    Per the task's exhaustive-grep instruction (sibling ^z3gn09m took 4 review rounds from non-exhaustive fixes), grepped the whole Sources/CodeContextKit/Search/ directory for `func .*(_ ` and found 5 more unlabeled-first-argument functions that violate the same "omit only for value-preserving conversions" rule, none of which are conversions:
    ...
    Re-ran the grep after fixing (`grep -rn "func .*(_ " Sources/CodeContextKit/Search/`) -> zero remaining matches.
