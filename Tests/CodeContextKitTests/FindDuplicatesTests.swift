...
/// `symbolPath`/`kind`/`text`/`embedding` values, which `FindDuplicatesOps`'
/// meta-type-partition and similarity-threshold tests need precise control
/// over. Mirrors `SearchCodeTests`' raw-SQL insert style (duplicated rather
/// than shared, since `SearchCodeTests`' own copy is file-private).
